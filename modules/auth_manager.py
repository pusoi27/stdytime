...
# Backward-compatible feature identifiers.
# Feature gating is currently not enforced; these constants remain only so
# existing decorators/usages continue importing without runtime errors.
...
def get_current_user_id():
    """Return the single local owner ID used by licensed desktop installs."""
    return 1
...
def get_user_by_id(user_id):
    ...
    try:
        ...
        c = conn.cursor()
        c.execute("SELECT id, email, role, is_active, must_change_password FROM users WHERE id = ?", (user_id,))
        row = c.fetchone()
        conn.close()
        if row:
            return User(row[0], row[1], row[2], bool(row[3]), bool(row[4]))
        return None
...
def get_user_by_email(email):
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT id, email, role, is_active, must_change_password FROM users WHERE email = ?", (email,))
        row = c.fetchone()
        conn.close()
        if row:
            return User(row[0], row[1], row[2], bool(row[3]), bool(row[4]))
